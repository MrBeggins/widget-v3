...
def _background_scheduled_loop():
    ...
    # Фоновый скан муверов ОТКЛЮЧЁН (2026-07-13): акции теперь открываются вместе с
    # фьючерсами — сравнивать нечего. Позже переделаем под небольшой список акций.
    # Чтобы включить обратно — верни, напр., {(6, 54)}.
    SCAN_TIMES   = set()       # пусто = скан не запускается
...
def _is_auction_time(instrument_type=None):
    ...
    - Фьючерсы (futures): 6:50-7:00 (открытие) — с 2026-07-14 вместе с акциями (было 8:50-9:00)
